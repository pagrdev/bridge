...
**3. Is this project registered?**

Pagr can only reach repositories that have been explicitly registered. Check the project list in
the `pagr status` output. If the current repository is missing, offer to register it:

```
pagr project add . --name <a short name the user will recognise>
```

Ask before running this — it grants Pagr access to this directory. Suggest a name based on the
...
whether this project is registered. If anything is still outstanding, say exactly which command
