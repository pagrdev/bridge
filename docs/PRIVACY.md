...
| `tmp/<handoffId>.ndjson` | a **plaintext dump of one Codex thread** (0600), written only when a handoff is being written by the receiving agent and that agent's context lives inside Codex's app-server rather than in a file on disk. It exists for the length of that one headless run, is read by an agent on this Mac and by nothing else, and is deleted afterwards whether the run succeeded, timed out, was cancelled or crashed. Never uploaded: no plaintext transcript leaves this Mac for a handoff. Claude sessions need no dump — their transcript is already a file, and it is read where it lies. |
